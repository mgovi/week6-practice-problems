...
def iterateDictionary(superheros):
  for x in superheros:
    print('real_name - ', x['real_name'], ', hero_name - ', x['hero_name'])
...
key='real_name'
def iterateDictionary2(superheros):
  for x in superheros:
    print(x[key])
iterateDictionary2(superheros)
...
def printMovieList(movie_collection):
  for k,v in movie_collection.items():
    print(len(v), k)
    for i in v:
      print(i)
    print("")
printMovieList(movie_collection)
#Expected Outpout
#7 DISNEY
#- Cinderella
#- Encanto
